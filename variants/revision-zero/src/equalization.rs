...
impl Equalization for Term {
    ...
    fn equalize<'a>(
        ...
        match (x.data.as_ref(), y.data.as_ref()) {
            ...
            (Term::Lam(_, x), Term::Lam(_, y)) => Self::equalize(x, y, ctx),
            ...
            => {
                Self::equalize(x0, y0, ctx) && Self::equalize(x1, y1, ctx)
            }
            (Term::BoolIntro(x), Term::BoolIntro(y)) => x == y,
            (Term::BoolElim(x0, x1, x2), Term::BoolElim(y0, y1, y2)) => {
                Self::equalize(x0, y0, ctx)
                    && Self::equalize(x1, y1, ctx)
                    && Self::equalize(x2, y2, ctx)
            }
            _ => {
                ctx.error(x.location.start, |builder| builder
                    ...
                    .with_label(Label::new((ctx.source_name(), x.location.clone()))
                        ...
                        .with_message(format!("this expression has WHNF: {}", x)))
                    .with_label(Label::new((ctx.source_name(), y.location.clone()))
                        .with_color(Color::Red)
                        .with_message(format!("... However, it is expected to equate with the expression here, whose WHNF is {}", y)))
                    .finish());
                false
            }
        }
